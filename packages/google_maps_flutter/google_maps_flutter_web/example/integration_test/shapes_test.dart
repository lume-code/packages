...
void main() {
  ...
  group('PolygonsController', () {
    ...
    testWidgets('addPolygons forwards editable to the gmaps Polygon', (
      WidgetTester tester,
    ) async {
      final polygons = <Polygon>{
        const Polygon(
      ...
      final gmaps.Polygon editable =
          controller.polygons[const PolygonId('editable')]!.polygon!;
    ...
    testWidgets('emits PolygonEditEvent with points and holes on edit', (
      ...
      final Future<PolygonEditEvent> received =
          events.stream.where((MapEvent<Object?> e) => e is PolygonEditEvent).cast<PolygonEditEvent>().first;
      final gmaps.Polygon gmPolygon =
          controller.polygons[const PolygonId('p')]!.polygon!;
    ...
    testWidgets('emits PolygonEditEvent reflecting hole mutations', (
      WidgetTester tester,
    ) async {
      controller.addPolygons(<Polygon>{
        const Polygon(
    ...
    testWidgets('does not emit PolygonEditEvent when not editable', (
      WidgetTester tester,
    ) async {
      controller.addPolygons(<Polygon>{
        const Polygon(
    ...
    testWidgets('changePolygons rewires listeners when editable is toggled on', (
      ...
      final Future<PolygonEditEvent> received =
          events.stream.where((MapEvent<Object?> e) => e is PolygonEditEvent).cast<PolygonEditEvent>().first;
      final gmaps.Polygon gmPolygon =
          controller.polygons[const PolygonId('t')]!.polygon!;
  ...
  group('PolylinesController', () {
    ...
    testWidgets('addPolylines forwards editable to the gmaps Polyline', (
      ...
      final gmaps.Polyline editable =
          controller.lines[const PolylineId('editable')]!.line!;
      final gmaps.Polyline notEditable =
          controller.lines[const PolylineId('not-editable')]!.line!;
    ...
    testWidgets('emits PolylineEditEvent on path mutation when editable', (
      ...
      final Future<PolylineEditEvent> received =
          events.stream.where((MapEvent<Object?> e) => e is PolylineEditEvent).cast<PolylineEditEvent>().first;
      final gmaps.Polyline line =
          controller.lines[const PolylineId('e')]!.line!;
    ...
    testWidgets('does not emit PolylineEditEvent when not editable', (
      WidgetTester tester,
    ) async {
      controller.addPolylines(<Polyline>{
        const Polyline(
          polylineId: PolylineId('n'),
          points: <LatLng>[LatLng(0, 0), LatLng(1, 1)],
        ),
      });
      ...
      final gmaps.Polyline line =
          controller.lines[const PolylineId('n')]!.line!;
      line.path.setAt(0, gmaps.LatLng(5, 5));
      await Future<void>.delayed(const Duration(milliseconds: 50));
    ...
    testWidgets('changePolylines rewires listeners when editable is toggled on', (
      ...
      final Future<PolylineEditEvent> received =
          events.stream.where((MapEvent<Object?> e) => e is PolylineEditEvent).cast<PolylineEditEvent>().first;
      final gmaps.Polyline line =
          controller.lines[const PolylineId('t')]!.line!;
